...
class DeviceScanActivity : AppCompatActivity() {
    ...
    override fun onRequestPermissionsResult(rc: Int, perms: Array<out String>, results: IntArray) {
        ...
        if (rc == RC_SCAN && results.isNotEmpty() && results.all { it == PackageManager.PERMISSION_GRANTED }) {
            statusText.text = "Permissions granted — tap a scan button"
            refreshPaired()
        } else if (rc == RC_SCAN) {
            statusText.text = "Bluetooth permissions are required to pair glasses"
            showPermissionRequiredState()
        }
    }
    ...
    private fun showPermissionRequiredState() {
        ...
        pairedContainer.addView(makeLabel("Bluetooth permission required", "#AA7777"))
    }
    ...
    private fun refreshPaired() {
        ...
        if (bonded.isEmpty()) {
            ...
            val sorted = bonded.sortedWith(
                compareByDescending<BluetoothDevice> { isLikelyRokidDevice(it) }
                    .thenBy { safeDeviceName(it)?.lowercase() ?: "" }
            )
            val rokidDevices = sorted.filter { isLikelyRokidDevice(it) }
            val savedMac = getPrefs().getString(KEY_GLASSES_MAC, null)
            if (savedMac == null && rokidDevices.size == 1) {
                saveGlassesInfo(rokidDevices.first())
                updateConnectedInfo()
                statusText.text = "Detected paired Rokid glasses — selected automatically"
            }
            for (d in sorted) {
                addDeviceRow(
                    pairedContainer,
                    d,
                    isRokid = isLikelyRokidDevice(d),
                    alreadyBonded = true
                )
            }
        }
    ...
    private fun isLikelyRokidDevice(device: BluetoothDevice): Boolean {
        val name = safeDeviceName(device)?.lowercase() ?: return false
        return name.contains("rokid") ||
