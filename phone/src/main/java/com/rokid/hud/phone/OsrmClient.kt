...
object OsrmClient {
    ...
    fun getRoute(fromLat: Double, fromLng: Double, toLat: Double, toLng: Double, routeMode: RouteMode): RouteResult {
        require(routeMode != RouteMode.TRANSIT) { "Transit mode requires Google routes." }
        val profile = routeMode.osrmProfile
        val url = URL("https://router.project-osrm.org/route/v1/$profile/$fromLng,$fromLat;$toLng,$toLat?overview=full&geometries=geojson&steps=true")
