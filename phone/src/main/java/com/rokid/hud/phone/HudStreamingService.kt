...
class HudStreamingService : Service() {
    ...
    companion object {
        ...
        const val ACTION_STOP_STREAMING = "com.rokid.hud.phone.action.STOP_STREAMING"
    ...
    var onClientConnectionChanged: ((Int) -> Unit)? = null
    ...
    fun connectedClientCount(): Int = clientSessions.size
    fun hasConnectedClient(): Boolean = clientSessions.isNotEmpty()
    ...
    private fun resendCachedState(writer: BufferedWriter) {
        try {
            cachedSettings?.let {
                writeLine(writer, ProtocolCodec.encodeSettings(it))
                Log.i(TAG, "Re-sent settings to new client")
            }
            cachedWifiCreds?.let {
                writeLine(writer, ProtocolCodec.encodeWifiCreds(it))
    ...
    private fun writeLine(writer: BufferedWriter, json: String) {
        synchronized(writer) {
            writer.write(json)
            writer.newLine()
            writer.flush()
    ...
    private fun notifyClientConnectionChanged() {
        val count = connectedClientCount()
        apkHandler.post { onClientConnectionChanged?.invoke(count) }
    }
