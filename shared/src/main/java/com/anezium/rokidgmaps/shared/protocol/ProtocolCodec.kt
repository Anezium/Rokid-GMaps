package com.anezium.rokidgmaps.shared.protocol
...
sealed class ParsedMessage {
    ...
    data class NavMode(val msg: NavModeMessage) : ParsedMessage()
    data class Pois(val msg: PoisMessage) : ParsedMessage()
...
object ProtocolCodec {
    ...
    fun encodeSettings(msg: SettingsMessage): String = JSONObject().apply {
        ...
        put(ProtocolConstants.FIELD_ROUTE_MODE, msg.routeMode)
        ...
        put(ProtocolConstants.FIELD_SHOW_LANDMARKS, msg.showLandmarks)
    }.toString()
    ...
    fun encodeNavMode(msg: NavModeMessage): String = JSONObject().apply {
        put(ProtocolConstants.FIELD_TYPE, ProtocolConstants.MessageType.NAV_MODE)
        put(ProtocolConstants.FIELD_PREVIEW_ACTIVE, msg.previewActive)
    }.toString()

    fun encodePois(msg: PoisMessage): String = JSONObject().apply {
        put(ProtocolConstants.FIELD_TYPE, ProtocolConstants.MessageType.POIS)
        put(ProtocolConstants.FIELD_POIS, JSONArray().apply {
            for (poi in msg.items) {
                put(JSONObject().apply {
                    put(ProtocolConstants.FIELD_NAME, poi.name)
                    put(ProtocolConstants.FIELD_LATITUDE, poi.latitude)
                    put(ProtocolConstants.FIELD_LONGITUDE, poi.longitude)
                    put(ProtocolConstants.FIELD_CATEGORY, poi.category)
                    put(ProtocolConstants.FIELD_PRIORITY, poi.priority)
                })
            }
        })
    }.toString()
            ...
            when (json.optString(ProtocolConstants.FIELD_TYPE)) {
                ...
                ProtocolConstants.MessageType.SETTINGS -> ParsedMessage.Settings(
                    SettingsMessage(
                        ...
                        routeMode = json.optString(ProtocolConstants.FIELD_ROUTE_MODE, "drive"),
                        ...
                        showLandmarks = json.optBoolean(ProtocolConstants.FIELD_SHOW_LANDMARKS, true)
                    )
                )
                ...
                ProtocolConstants.MessageType.NAV_MODE -> ParsedMessage.NavMode(
                    NavModeMessage(
                        previewActive = json.optBoolean(ProtocolConstants.FIELD_PREVIEW_ACTIVE, false)
                    )
                )
                ProtocolConstants.MessageType.POIS -> {
                    val poisArr = json.optJSONArray(ProtocolConstants.FIELD_POIS) ?: JSONArray()
                    val items = (0 until poisArr.length()).mapNotNull { i ->
                        val item = poisArr.optJSONObject(i) ?: return@mapNotNull null
                        val lat = item.optDouble(ProtocolConstants.FIELD_LATITUDE, Double.NaN)
                        val lng = item.optDouble(ProtocolConstants.FIELD_LONGITUDE, Double.NaN)
                        if (!lat.isFinite() || !lng.isFinite()) return@mapNotNull null
                        PoiInfo(
                            name = item.optString(ProtocolConstants.FIELD_NAME, ""),
                            latitude = lat,
                            longitude = lng,
                            category = item.optString(ProtocolConstants.FIELD_CATEGORY, ""),
                            priority = item.optInt(ProtocolConstants.FIELD_PRIORITY, 0)
                        )
                    }
                    ParsedMessage.Pois(PoisMessage(items))
                }
