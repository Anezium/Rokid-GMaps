...
data class HudState(
    ...
    val contentView: HudContentView = HudContentView.MAP,
    ...
    fun toggleContentView(): HudState {
        if (!hasTransitContent()) return this
        return copy(
            contentView = when (contentView) {
                HudContentView.MAP -> HudContentView.TRANSIT_RECAP
                HudContentView.TRANSIT_RECAP -> HudContentView.MAP
            }
        )
    }
}
